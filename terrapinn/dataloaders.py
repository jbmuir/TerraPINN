...
def uniform_plus_kde_sampler(rng_key, ndims, weights=None, old_coords=None, hbatch_size=10000, bandwidth=0.5):
    ...
    if weights == None:
        ...
        kde = KernelDensity(kernel='tophat', bandwidth=bandwidth)
        #transform old time coordinate from [0,1] to [-1,1]
        old_coords = jnp.hstack(old_coords)
        old_coords = old_coords.at[:,0].multiply(2.0)
        old_coords = old_coords.at[:,0].add(-1.0)
    ...
    samples = samples.at[:,0].divide(2.0)
